...
4-2. [Characteristic 4 — 페어링(마스터 MAC)](#4-2-characteristic-4--페어링마스터-mac-수신)
5. [타이밍](#5-타이밍)
6. [연결 파라미터](#6-연결-파라미터)
7. [채널-핀 매핑](#7-채널-핀-매핑)
...
| Characteristic UUID (페어링) | `abcd1234-5678-1234-5678-abcdef123459` |
...
| 페어링 (`...59`) | WRITE + READ + NOTIFY | BLE2902 포함. READ로 저장된 MAC 조회 가능 (§4-2) |
...
## 4-2. Characteristic 4 — 페어링(마스터 MAC) 수신

마스터(PC 설정 프로그램 또는 스마트폰 앱)가 **로봇 의수의 BLE MAC 주소**를
암밴드에 등록하기 위한 Characteristic. 설계 흐름은 `docs/step_0.png` 참고.

### 설계 배경

쓰기 가능한 Characteristic이 기존에는 가중치 수신(`...58`) 하나뿐이었는데,
거기에 페어링 패킷을 얹으면 **53KB를 219회에 나눠 받는 상태기계가 오염된다**
(매직넘버 검사에서 걸려 `ERR:SIZE`를 회신하고 버퍼가 리셋됨). 그래서 전용
Characteristic을 별도로 팠다.

READ 속성을 넣은 이유는 **저장 여부를 클라이언트가 확인할 수 있게** 하기
위함이다. 쓰기 성공이나 응답 수신만으로는 펌웨어가 실제로 저장했는지 알 수 없다.

### 식별

| 항목 | 값 |
|------|----|
| Characteristic UUID | `abcd1234-5678-1234-5678-abcdef123459` |
| 속성 | **WRITE**(마스터 → 암밴드) + **READ**(저장값 조회) + **NOTIFY**(결과 응답) |
| CCCD | 포함 |
| 저장 위치 | NVS(`Preferences`), 네임스페이스 `pairing`, 키 `master_mac` |

### 패킷 포맷 (마스터 → 암밴드)

```
[1 byte] HDR      : 0xE0
[6 byte] MAC      : 로봇 의수의 BLE MAC (정순, 예: 3C A5 51 99 BB 5F)
[1 byte] CHECKSUM : byte 1~6의 XOR
```

- 총 8 bytes, 한 번의 Write로 전송(청크 분할 없음)
- 예: `E0 3C A5 51 99 BB 5F B5`
- 바이트 순서는 **정순**(사람이 읽는 순서 그대로). 펌웨어는 받은 6byte를 그대로
  저장하므로, 순서를 바꾸려면 마스터 쪽에서 뒤집어 보내면 된다.

### 암밴드 응답 (NOTIFY)

| 응답(UTF-8 텍스트) | 의미 |
|---|---|
| `OK:PAIR` | 검증 통과, NVS 저장 완료 |
| `ERR:PAIR_FORMAT` | 길이가 8이 아니거나 헤더가 `0xE0`이 아님 |
| `ERR:PAIR_CRC` | 체크섬 불일치 |

### 저장값 조회 (READ)

READ하면 저장된 MAC **6 bytes**가 그대로 돌아온다. 저장된 적이 없으면 **길이 0**.

```
READ ...59  →  3C A5 51 99 BB 5F
```

`onRead()` 콜백에서 매번 현재 값을 채우므로, 직전에 NOTIFY로 응답 문자열을
보냈더라도 READ는 항상 MAC을 돌려준다(READ와 NOTIFY가 값 버퍼를 공유하기 때문에
필요한 처리다).

### 부팅 시 동작

`setup()`에서 NVS를 읽어 복원하고 시리얼에 남긴다.

```
# PAIR: 저장된 마스터 3C:A5:51:99:BB:5F
# PAIR: 저장된 마스터 없음
```

첫 부팅에는 `pairing` 네임스페이스가 없어 `nvs_open failed` 류의 로그가 함께
뜰 수 있으나 무해하다.

### 구현 시 주의

- **`getValue()` 대신 `getData()`/`getLength()`를 쓸 것.** MAC에는 `0x00`이
  포함될 수 있는데(예: `3C:00:51:...`), Arduino `String`은 널 종료 문자열이라
  중간에서 잘릴 수 있다.
- `onWrite()` 안에서 블로킹 호출(`delay()`, `ESP.restart()`)을 하지 말 것 —
  §4-1의 주의사항과 동일한 이유로 ATT 확인 응답이 나가지 못한다.

### 미구현 — 저장한 MAC의 용도

현재 펌웨어는 **MAC을 저장하기만 하고 사용하지 않는다.** `docs/step_3.png`의
운용 단계(의수가 마스터로 암밴드에 접속)를 구현하려면 다음이 추가로 정해져야 한다.

- 저장한 MAC을 **접속 허용 목록(화이트리스트)** 으로 쓸 것인지
- 아니면 의수 쪽에도 **암밴드 MAC을 전달**해야 하는지
  (의수가 마스터로 접속하려면 암밴드의 주소를 알아야 한다)
